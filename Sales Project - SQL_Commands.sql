...
-->Analysis - The most costliers products in the product list are laptops from HP and Dell as there cost is greater than Rs 50,000.

---3 HOW MANY DELIVERY PERSON ARE THERE IN EACH PINCODE?
SELECT COUNT(*) AS DELI_PERSON,pincode
FROM delivery_person
GROUP BY pincode;

-->Analysis - Out of total 6 pincode, pincode- 400001 has maximum number of delivery person i.e. 4 and pincode- 700001 has second maximum number of delivery person i.e. 2. All other pincodes have only 1 delivery person.

--- 5 For each Pin code, print the count of orders, sum of total amount paid, average amount paid, 
--maximum amount paid, Minimum amount paid for the transactions which were paid by 'cash'. Take only 
--'buy' order types 
SELECT
    delivery_pincode,
...
--->> delivery pincode 400001 and count of order 105 and total amount is 11546300 where as pincode 600001 and 
---->count of order is 19 and total amount is paid 76647 which very low and need to work at that particular area pincode 
...
--->> Analysis - Delivery person Simon williamswith delivery id - 1000002 has delivered the highest number of order i.e. 10 and the total amount for all the ordered delievered by him is 76801
...
-->Analysis - There are 8 customers whose email is a gmail account.
...
ORDER BY
    Order_Month;
-->>Analysis - In 5th month (may) we have maximum number of orders which is 125 and total returned order in this month is 8. 
...
-->> Analysis - There are 2 brands HP and Dell. Dell has sold a total of 502 units out of which 21 units were returned. HP has sold a total of 498 units out of which 29 units were returned.

-->12 12.	How many distinct customers and delivery boys are there in each state? 
...
GROUP BY
    p.category
ORDER BY
    Absolute_Profit DESC, Percentage_Profit DESC;

-->> Analysis - Percentage wise pendrive has made the highest profit as the profit percent is highest for it. In case of absolute profit, it is laptop that has made the highest profit.
...
WHERE
    product_name = 'Dell AX420' AND o.order_type = 'Buy'
GROUP BY
    tot_units
ORDER BY
    tot_units ;

 -->> aas per quetions discount percent is very frequent because no of orders
 --by order id / person are and discount percent also incerasing by one percent  
...
WITH MonthlyOrders AS (
    SELECT
        ...
        COUNT(CASE WHEN o.order_type != 'Return' THEN o.order_id END) AS OrderCount
    FROM
        delivery_person AS dp
    LEFT JOIN
        orders AS o
    ON
        dp.delivery_person_id = o.delivery_person_id
    GROUP BY
        dp.name, DATEPART(Month, o.order_date)
)
...
-->>Analysis - In November and december there are zero buy orders for all the delivery persons.
